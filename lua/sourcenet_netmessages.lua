...
-- svc_VoiceData fix provided for free by time from garnetgaming, enjoy :)
	...
	SVC = {
		...
		[svc_VoiceData] = { -- 15
			DefaultCopy = function(netchan, read, write)
				...
				if bits > 0 then -- this is where normal voice messages of > 0 bytes are broadcasted to clients
					local voicedata = read:ReadBits(bits)
					write:WriteBits(voicedata)
				else -- this is where we handle a svc_VoiceData message of 0 bytes
					-- get player by adding 1 to their client number
					local ply = ents.GetByIndex(client + 1)
					
					-- initialize player's limiter (we don't want to ban based off just one instance)
					...
					elseif now > limit.firsttime + 2 then
						...
						limit.lasttime = now
						limit.calls = 1
					-- if limit of 1000 not yet reached, increment limit.calls by 1
					elseif limit.calls < 1000 then
						limit.calls = limit.calls + 1
						limit.lasttime = now		
					else -- more than 1000 instances of svc_VoiceData 0 in past 2 seconds
						-- save crasher's steamid
						local crasher_steamid = ply:SteamID()
						
						-- immediately disconnect player from the server
						ply:Kick("no pls")
						
						if not limit.banned then
							-- ban crasher_steamid here if you wish --
							--
							
							limit.banned = true -- important, or you may end up banning them 5 or 6 times due to the speed at which
